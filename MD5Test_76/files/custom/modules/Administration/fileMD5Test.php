...
class MD5FileValidator
{
    ...
    public function createScripts()
    {
        ...
        if (!empty($this->result['changed']) || !empty($this->result['missing'])) {
            ...
            $newManifest[] = "    ),\n);\n?>";
            $scriptFileName = $filePath . "/manifest.php";
            file_put_contents($scriptFileName, implode("", $newManifest), LOCK_EX);
            $zip->addFile($scriptFileName, basename($scriptFileName));
            unlink($scriptFileName);
            ...
            $scriptFileName = $filePath . "/manifest_complier.php";
            file_put_contents($scriptFileName, $compiler, LOCK_EX);
            $zip->addFile($scriptFileName, basename($scriptFileName));
            unlink($scriptFileName);
            $zip->close();
        }
